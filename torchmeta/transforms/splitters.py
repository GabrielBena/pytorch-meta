...
class Splitter(object):
    ...
    def get_indices(self, task):
        if isinstance(task, ConcatTask):
            ...
            raise ValueError(
                "The task must be of type `ConcatTask` or `Task`, " "Got type `{0}`.".format(type(task))
            )
        return indices

    def get_indices_task(self, task):
        raise NotImplementedError(
            "Method `get_indices_task` must be " "implemented in classes inherited from `Splitter`."
        )
...
class ClassSplitter_(Splitter):
    ...
    def get_indices_task(self, task):
        ...
        for i, (name, class_indices) in enumerate(all_class_indices.items()):
            ...
            if self.shuffle:
                seed = (hash(task) + i + self.random_state_seed) % (
                    2**32
                ) + self.hard_shuffle * np.random.randint(0, 1000)
        ...
        for dataset in task.datasets:
            ...
            if num_samples < self._min_samples_per_class:
                raise ValueError(
                    ...
                    "is smaller than the minimum number of samples per class "
                    "required by `ClassSplitter` ({1}).".format(num_samples, self._min_samples_per_class)
                )
            ...
            if self.shuffle:
                ...
                dataset_indices = np.arange(num_samples)
            # print(dataset_indices.shape, dataset_indices[:10], dataset_indices[-10:])
